...
export default function MerchantAuthPage() {
  ...
  const { registerMerchant, loading, logout } = useAuth()
  const { toast } = useToast()
  const [step, setStep] = useState(1)

  // Clear any existing session when accessing merchant auth page
  useEffect(() => {
    logout()
  }, [logout])
    ...
    try {
      ...
      if (error.message) {
        if (error.message.includes("User already exists")) {
          ...
          errorMessage = "Please enter a valid Algorand wallet address"
        } else {
